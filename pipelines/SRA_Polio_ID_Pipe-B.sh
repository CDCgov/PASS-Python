...
#$ -pe smp 4-16 # number of cores to use from 4-16
...
if [ "$HAVEIT" == 0 ]; then
    ...
    if [ -s $SAMP.Polio.fq ]; then
        ...
        mkdir SPAdes_"$SAMP"
        spades.py -t 16 -s "$SAMP".Polio.fq -o SPAdes_"$SAMP"
        cd SPAdes*
        ...
        date2=$(date +"%s")
        diff=$(($date2 - $date1))
        mongo --quiet ncbs-dev-05.biotech.cdc.gov/PASS --eval 'db.data.findOneAndUpdate({"Run":"'${SAMP}'"},{$set:{"Run": "'$SAMP'","DateProcessed":"'$dateStart'"}},{upsert: true})'
        mongo --quiet ncbs-dev-05.biotech.cdc.gov/PASS --eval 'db.data.findOneAndUpdate({"Run":"'${SAMP}'"},{$set:{"Run": "'$SAMP'","DateStart":"'$date1'"}},{upsert: true})'
        mongo --quiet ncbs-dev-05.biotech.cdc.gov/PASS --eval 'db.data.findOneAndUpdate({"Run":"'${SAMP}'"},{$set:{"Run": "'$SAMP'","DateEnd":"'$date2'"}},{upsert: true})'
        mongo --quiet ncbs-dev-05.biotech.cdc.gov/PASS --eval 'db.data.findOneAndUpdate({"Run":"'${SAMP}'"},{$set:{"Run": "'$SAMP'","ElapsedTimeSec":"'$diff'"}},{upsert: true})'

    else #no potential polio reads
        echo "FILE EMPTY"
        ...
        # remove file sra file and folder
        rm -r $OUTDIR/$SAMP
    fi
